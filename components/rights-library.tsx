...
type RightsLibraryProps = Pick<PublicRightsData, "roles" | "rules" | "topics" | "dataSource">;

function buildRuleCounts(roles: PublicRole[], rules: PublicRule[]) {
  return new Map(roles.map((item) => [
    item.id,
    rules.reduce((count, rule) => count + Number(rule.roles.includes(item.id)), 0),
  ]));
}

export function RightsLibrary({ roles, rules, topics, dataSource }: RightsLibraryProps) {
  ...
  }), [normalizedQuery, role, rules, topic]);
