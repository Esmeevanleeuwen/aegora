...
func main() {
    ...
    do {
        ...
        if let existingPkg = rootObject.remotePackages.first(where: { $0.repositoryURL == repoURL }) {
            ...
            packageRef.versionRequirement = .upToNextMajorVersion(versionRequirementString)
            print("Package already present, updating version requirement.")
        } else {
        ...
        if products.contains(where: { $0.contains("CSCrashReporter")}) {
